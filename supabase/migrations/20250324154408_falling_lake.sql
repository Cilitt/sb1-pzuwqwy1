...
-- Create media_tags table (since it's referenced by media_files_tags)
...
DO $$
BEGIN
    -- Check and create policy for media_categories
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'media_categories' 
    ...
    -- Check and create policy for media_tags
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'media_tags' 
    ...
    -- Check and create policy for media_files
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'media_files' 
        AND policyname = 'Users can manage their own media files'
    ) THEN
    ...
    -- Check and create policy for media_files_tags
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'media_files_tags' 
        AND policyname = 'Users can manage their own file tags'
    ) THEN
...
DO $$
BEGIN
    -- Check and create trigger for media_categories
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
        WHERE tgname = 'update_media_categories_updated_at'
    ) THEN
    ...
    -- Check and create trigger for media_files
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger 
